...
<?php $form = ActiveForm::begin(); ?>
    ...
    <form action="surat-sptgr.php" method="post">
        ...
        <?= $form->field($model, 'tglrapat')->widget(DatePicker::classname(), [
            'pluginOptions' => [
            'format' => 'yyyy-mm-dd',
            'todayHighlight' => true
        ]
        ]); ?>
        <?= $form->field($model, 'nmrberitaacara') ?>
        <?= $form->field($model, 'thnberitaacara') ?>
        <?= $form->field($model, 'jmlunit') ?>
        <?= $form->field($model, 'barang') ?>
        <?= $form->field($model, 'harihilang') ?>
        <?= $form->field($model, 'tglhilang')->widget(DatePicker::classname(), [
            'pluginOptions' => [
            'format' => 'yyyy-mm-dd',
            'todayHighlight' => true
        ]
        ]); ?>
        <?= $form->field($model, 'tempathilang') ?>
        <?= $form->field($model, 'nilai') ?>
        <?= $form->field($model, 'dasarharga') ?>
